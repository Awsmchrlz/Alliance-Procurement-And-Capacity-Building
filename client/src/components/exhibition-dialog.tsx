...
interface FormDataType {
  ...
  exhibitorType: 'institutional' | 'individual' | '';
...
export function ExhibitionDialog({ event, open, onOpenChange, onSuccess }: ExhibitionDialogProps) {
  ...
  const validateStep = (step: number) => {
    if (step === 1) {
      ...
      if (!formData.exhibitorType) return { isValid: false, message: 'Please select an exhibitor package type' };
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const exhibitionData = {
        ...
        boothSize: 'standard',
        amount: formData.exhibitorType === 'institutional' ? EXHIBITION_PACKAGES.institutional.priceUSD : EXHIBITION_PACKAGES.individual.priceUSD,
        currency: 'USD',
  ...
  const renderStep1 = () => (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <Label htmlFor="exhibitorType" className="text-sm font-medium text-gray-700">Exhibitor Package *</Label>
        <select
          id="exhibitorType"
          value={formData.exhibitorType}
          onChange={(e) => updateField('exhibitorType', e.target.value)}
          className="w-full h-12 sm:h-11 px-4 text-base sm:text-sm border border-gray-200 rounded-xl focus:ring-2 focus:ring-red-600 focus:border-red-600 transition-all bg-white"
        >
          <option value="">Select a package...</option>
          <option value="institutional">{EXHIBITION_PACKAGES.institutional.name} - USD {EXHIBITION_PACKAGES.institutional.priceUSD}</option>
          <option value="individual">{EXHIBITION_PACKAGES.individual.name} - USD {EXHIBITION_PACKAGES.individual.priceUSD}</option>
        </select>
      </div>
  ...
  const renderStep2 = () => {
    const currentPackage = formData.exhibitorType === 'institutional' ? EXHIBITION_PACKAGES.institutional : EXHIBITION_PACKAGES.individual;
      ...
      <div className="space-y-6">
        ...
        <div className="p-4 sm:p-6 border-2 border-red-200 bg-red-50 rounded-xl">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-red-600">
                  USD {currentPackage?.priceUSD.toLocaleString()}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2 mt-4">
            <h5 className="text-sm font-medium text-gray-700 mb-2">Package Benefits:</h5>
        ...
        <div className="p-4 bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg">
          ...
          <p className="text-sm text-green-700">
            You've selected the <strong>{currentPackage?.name}</strong> for <strong>USD {currentPackage?.priceUSD.toLocaleString()}</strong>. 
      ...
      <div className="space-y-6">
        ...
        <div className="bg-red-50 p-6 rounded-2xl border border-red-100">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h4 className="font-semibold text-gray-900">{currentPackage?.name || "Exhibition Package"}</h4>
              <p className="text-lg font-bold text-red-700">
                USD {currentPackage?.priceUSD.toLocaleString() || "0"}
              </p>
            </div>
          </div>
          
          <div className="mt-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
            <h5 className="font-medium text-gray-900 mb-3 text-sm uppercase tracking-wider">Order Summary</h5>
            <div className="flex justify-between items-center py-2 border-b border-gray-200">
              <span className="text-sm text-gray-700">{currentPackage?.name || "Package"}</span>
